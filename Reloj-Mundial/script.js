...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function playBeep(frecuencia = 880, duracion = 0.1, tipo = 'square') {
    if (!audioCtx) return;
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = tipo;
  ...
  function sonidoAgregar() {
    playBeep(660, 0.08);
    setTimeout(() => playBeep(880, 0.1), 80);
  }

  function sonidoEliminar() {
    playBeep(440, 0.1);
    setTimeout(() => playBeep(330, 0.12), 80);
  }

  function sonidoCambioTema() {
    playBeep(1200, 0.05);
    setTimeout(() => playBeep(1000, 0.05), 60);
  }

  document.body.addEventListener("click", iniciarSonido, { once: true });
  ...
  function renderizarRelojes() {
    ...
    misCiudades.forEach(ciudad => {
      ...
      tarjeta.innerHTML = `
        ...
        <div class="clima clima-cargando" id="clima-${ciudad.replace(/\s/g, '')}">🌡️ --</div>
      `;
      ...
      // Obtener clima y actualizar
      window.obtenerClima(ciudad).then(datos => {
        const climaDiv = document.getElementById(`clima-${ciudad.replace(/\s/g, '')}`);
        if (climaDiv && datos.emoji) {
          climaDiv.classList.remove("clima-cargando");
          climaDiv.textContent = `${datos.emoji} ${datos.temp}`;
        }
      });
    });
  ...
  if (btnA) {
    btnA.addEventListener("click", () => playBeep(440, 0.1));
  }
  if (btnB) {
    btnB.addEventListener("click", () => playBeep(587, 0.1));
  }
